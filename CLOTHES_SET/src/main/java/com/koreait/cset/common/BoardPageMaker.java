...
public class BoardPageMaker {
	...
	public static String getPageView(String path, int page, int recordPerPage, int totalRecord, int bClass) {
		...
		sb.append("<div class='page-block'>");
		if ( beginPageOfBlock < pagePerBlock ) {
			sb.append("<span><i class='fas fa-angle-left' style='color: lightgray;'></i></span>&nbsp;&nbsp;");
		} else {
			sb.append("<a href='" + path + "?page=" + (beginPageOfBlock - 1) + "&bClass=" + bClass + "'><i class='fas fa-angle-left'></i></a>&nbsp;&nbsp;");
		}
		...
		for ( int p = beginPageOfBlock; p <= endPageOfBlock; p++ ) {
			if ( p == page ) {
				...
				sb.append("<a href='" + path + "?page=" + p + "&bClass=" + bClass + "'>" + p + "</a>&nbsp;&nbsp;");
			}
		}
		...
		if ( endPageOfBlock == totalPage ) {
			sb.append("<span><i class='fas fa-angle-right' style='color: lightgray;'></i></span>");
		} else {
			sb.append("<a href='" + path + "?page=" + (endPageOfBlock + 1) + "&bClass=" + bClass + "'><i class='fas fa-angle-right'></i></a>");
		}
		sb.append("</div>");
